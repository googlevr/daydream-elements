...
Shader "DaydreamElements/Demo/Combine"
{
  Properties
  {
  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      fixed4 frag (v2f i) : SV_Target
      {
        ...
        float color2 = DecodeFloatRGBA(tex2D (_MainTex2, i.uv20));
        color2 += DecodeFloatRGBA(tex2D (_MainTex2, i.uv21));
        color2 += DecodeFloatRGBA(tex2D (_MainTex2, i.uv22));
        color2 += DecodeFloatRGBA(tex2D (_MainTex2, i.uv23));
        float col2 = color2 *.25;
        return EncodeFloatRGBA(min(col,col2));
        */
